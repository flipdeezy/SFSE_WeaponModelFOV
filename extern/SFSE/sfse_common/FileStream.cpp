...
bool FileStream::open(const char *path) { return internalOpen(path, "rb"); }

bool FileStream::open(const wchar_t *path) { return internalOpen(path, L"rb"); }

bool FileStream::create(const char *path) { return internalOpen(path, "wb"); }
...
void FileStream::flush() { fflush(m_file); }
