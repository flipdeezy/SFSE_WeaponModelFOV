...
// this has been tested to work for non-varargs functions
// varargs functions end up with 'this' passed as the last parameter (ie.
// probably broken) do NOT use with classes that have multiple inheritance

// if many member functions are to be declared, use MEMBER_FN_PREFIX to create a
// type with a known name so it doesn't need to be restated throughout the
// member list

// all of the weirdness with the _GetType function is because you can't declare
// a static const pointer inside the class definition. we sadly can't inline
// anymore because of relocation.

// RelocPtr only works at a global scope, which we can't handle or we'd be
// bypassing the function route altogether
...
#define DEFINE_MEMBER_FN(functionName, retnType, address, ...)                 \
  DEFINE_MEMBER_FN_LONG(_MEMBER_FN_BASE_TYPE, functionName, retnType, address, \
                        __VA_ARGS__)
...
#define DEFINE_MEMBER_FN_0(fnName, retnType, addr)                             \
  ...
  FORCE_INLINE retnType fnName() {                                             \
    struct empty_struct {};                                                    \
    typedef retnType (empty_struct::*_##fnName##_type)();                      \
    const static uintptr_t address =                                           \
        fnName##_Address + RelocationManager::s_baseAddr;                      \
    _##fnName##_type fn = *(_##fnName##_type *)&address;                       \
    return (reinterpret_cast<empty_struct *>(this)->*fn)();                    \
  }
#define DEFINE_MEMBER_FN_1(fnName, retnType, addr, ...)                        \
  static const std::uintptr_t fnName##_Address = addr;                         \
  template <typename T1> FORCE_INLINE retnType fnName(T1 &&t1) {               \
    struct empty_struct {};                                                    \
    typedef retnType (empty_struct::*_##fnName##_type)(__VA_ARGS__);           \
    const static uintptr_t address =                                           \
        fnName##_Address + RelocationManager::s_baseAddr;                      \
...
// this is the solution to getting a pointer-to-member-function pointer
template <typename T> uintptr_t getFnAddr(T src) {
  union {
    uintptr_t u;
    T t;
  } data;
...
const void *getResourceLibraryProcAddress(const void *module,
