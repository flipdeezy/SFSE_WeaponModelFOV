...
void DebugLog::openRelative(int folderID, const char *relPath) {
  ...
  HRESULT err = SHGetFolderPath(NULL, folderID | CSIDL_FLAG_CREATE, NULL,
                                SHGFP_TYPE_CURRENT, path);
  if (!SUCCEEDED(err)) {
    _FATALERROR(
        "Your virus scanner is blocking access to your My Documents folder. "
        "SHGetFolderPath %08X failed (result = %08X lasterr = %08X)",
        folderID, err, GetLastError());
  }
