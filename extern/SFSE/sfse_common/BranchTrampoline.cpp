...
BranchTrampoline::~BranchTrampoline() { destroy(); }
...
bool BranchTrampoline::create(size_t len, void *module) {
  ...
  uintptr_t maxDisplacement =
      0x80000000 -
      (1024 * 1024 *
       128); // largest 32-bit displacement with 128MB scratch space
  uintptr_t lowestOKAddress =
      (moduleBase >= maxDisplacement) ? moduleBase - maxDisplacement : 0;
  addr--;
  ...
  while (!m_base) {
    ...
    if (info.State == MEM_FREE) {
      ...
      if (info.RegionSize >= len) {
        ...
        m_base =
            (void *)VirtualAlloc((void *)addr, len, MEM_COMMIT | MEM_RESERVE,
                                 PAGE_EXECUTE_READWRITE);
        if (m_base) {
          m_len = len;
          m_allocated = 0;
        } else {
          _WARNING("trampoline alloc %016I64Xx%016I64X failed (%08X)", addr,
                   len, GetLastError());
        }
      }
    }
...
bool BranchTrampoline::write6Branch_Internal(uintptr_t src, uintptr_t dst,
                                             u8 op) {
  bool result = false;
...
bool BranchTrampoline::write5Branch_Internal(uintptr_t src, uintptr_t dst,
                                             u8 op) {
  bool result = false;
...
#pragma pack(pop)
  ...
  TrampolineCode *trampolineCode =
      (TrampolineCode *)allocate(sizeof(TrampolineCode));
  if (trampolineCode) {
    trampolineCode->Init(dst);
