...
public:
  RelocAddr(uintptr_t offset)
      : m_offset(reinterpret_cast<BlockConversionType *>(
            offset + RelocationManager::s_baseAddr)) {
    //
  }
...
private:
  ...
  // that's kind of stupid and makes it impossible to use this for uintptr_ts if
  // I use the same type so we make a new type by storing the data in a pointer
  // to this useless struct at least this is hidden by a wrapper
