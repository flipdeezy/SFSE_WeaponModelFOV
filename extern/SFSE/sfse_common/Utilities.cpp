...
const std::string &getRuntimeDirectory() {
  ...
  if (s_runtimeDirectory.empty()) {
    ...
    if (lastSlash !=
        std::string::npos) // if we don't find a slash something is VERY WRONG
    {
...
std::string getConfigOption(const char *section, const char *key) {
  ...
  if (!configPath.empty()) {
    ...
    u32 resultLen = GetPrivateProfileString(
...
#pragma warning(disable : 4996)
    if (GetVersionEx(&info)) {
      ...
      sprintf_s(buf, sizeof(buf), "%d.%d (%d)", info.dwMajorVersion,
...
const void *getResourceLibraryProcAddress(const void *module,
  ...
  auto *exportAddresses =
      (const u32 *)(base + exportTable->AddressOfFunctions); // RVA array
  auto *exportNameOrdinals =
      (const u16 *)(base +
                    exportTable
                        ->AddressOfNameOrdinals); // index in to exportNames
  auto *exportNames =
      (const u32 *)(base + exportTable->AddressOfNames); // RVA array
