...
Options::Options()
    : m_launchCS(false), m_setPriority(false), m_priority(0), m_crcOnly(false),
      m_optionsOnly(false), m_waitForClose(false), m_verbose(false),
      m_moduleInfo(false), m_skipLauncher(true), m_launchSteam(false),
      m_noTimeout(false), m_forceSteamLoader(false), m_affinity(0) {
  //
}
...
void Options::PrintUsage(void) {
  ...
  _MESSAGE("  -minfo - log information about the DLLs loaded in to the target "
           "process");
  _MESSAGE(
      "  -noskiplauncher - does not skip the default Bethesda launcher window");
  _MESSAGE("                    note: specifying this option may cause "
