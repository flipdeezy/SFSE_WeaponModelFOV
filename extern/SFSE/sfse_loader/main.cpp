...
int main(int argc, char **argv) {
  DebugLog::openRelative(CSIDL_MYDOCUMENTS, "\\My Games\\" SAVE_FOLDER_NAME
                                            "\\SFSE\\Logs\\sfse_loader.txt");
  // gLog.SetPrintLevel(IDebugLog::kLevel_FatalError);
  // gLog.SetLogLevel(IDebugLog::kLevel_DebugMessage);
  ...
  _MESSAGE("SFSE loader: initialize (version = %d.%d.%d %08X %04d-%02d-%02d "
           "%02d:%02d:%02d, os = %s)",
           SFSE_VERSION_INTEGER, SFSE_VERSION_INTEGER_MINOR,
           SFSE_VERSION_INTEGER_BETA, RUNTIME_VERSION, now.wYear, now.wMonth,
  ...
  if (g_options.m_launchCS) {
    ...
    if (!procName.empty()) {
      ...
      std::string appName = getRuntimeName();
      if (!_stricmp(appName.c_str(), procName.c_str())) {
        PrintLoaderError("You have renamed sfse_loader and have not specified "
  ...
  // check if the exe exists
  {
    ...
    if (!fileCheck.open(procPath.c_str())) {
      if (usedCustomRuntimeName) {
        PrintLoaderError(
            "Couldn't find %s. You have customized the runtime name via SFSE's "
            ".ini file, and that file does not exist. This can usually be "
            "fixed by removing the RuntimeName line from the .ini file.)",
            procName.c_str());
      } else {
        PrintLoaderError("Couldn't find %s.", procName.c_str());
      }
  ...
  // check exe version
  if (!IdentifyEXE(procPath.c_str(), g_options.m_launchCS, &dllSuffix,
                   &procHookInfo)) {
    _ERROR("unknown exe");

    if (usedCustomRuntimeName) {
      PrintLoaderError("You have customized the runtime name via SFSE's .ini "
                       "file. Version errors can usually be fixed by removing "
                       "the RuntimeName line from the .ini file.");
    }
  ...
  // check to make sure the dll exists
  {
    ...
    if (!tempFile.open(dllPath.c_str())) {
      PrintLoaderError(
          "Couldn't find SFSE DLL (%s). Please make sure you have installed "
          "SFSE correctly and are running it from your Starfield folder.",
          dllPath.c_str());
      return 1;
    }
  }
  ...
  if (!CreateProcess(procPath.c_str(),
                     ...
                     &startupInfo, &procInfo)) {
    if (GetLastError() == 740) {
      PrintLoaderError("Launching %s failed (%d). Please try running "
                       "sfse_loader as an administrator.",
                       procPath.c_str(), GetLastError());
    } else {
      PrintLoaderError("Launching %s failed (%d).", procPath.c_str(),
                       GetLastError());
    }
  ...
  case kProcType_Normal:
  case kProcType_GOG:
    injectionSucceeded = InjectDLLThread(&procInfo, dllPath.c_str(), true,
                                         g_options.m_noTimeout);
    break;
  ...
  if (!injectionSucceeded) {
    ...
    if (!ResumeThread(procInfo.hThread)) {
      _WARNING("WARNING: something has started the runtime outside of "
               "sfse_loader's control.");
      _WARNING("SFSE will probably not function correctly.");
      _WARNING("Try running sfse_loader as an administrator, or check for "
               "conflicts with a virus scanner.");
    }
