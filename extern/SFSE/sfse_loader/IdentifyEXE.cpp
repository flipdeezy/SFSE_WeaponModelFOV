...
static bool GetFileVersion(const char *path, VS_FIXEDFILEINFO *info,
  ...
  if (versionBuf) {
    if (GetFileVersionInfo(path, NULL, versionSize, versionBuf)) {
      ...
      if (VerQueryValue(versionBuf, "\\", (void **)&retrievedInfo,
                        (PUINT)&realVersionSize) &&
          retrievedInfo) {
        *info = *retrievedInfo;
        result = true;
      } else {
      ...
      if (outProductName) {
        ...
        if (VerQueryValue(versionBuf, "\\StringFileInfo\\040904B0\\ProductName",
                          (void **)&productName, (PUINT)&productNameLen) &&
            productNameLen && productName) {
...

      {
        ...
        if (VerQueryValue(
                versionBuf, "\\StringFileInfo\\040904B0\\ProductVersion",
                (void **)&productVersion, (PUINT)&productVersionLen) &&
...
static bool VersionStrToInt(const std::string &verStr, u64 *out) {
  ...
  if (sscanf_s(verStr.c_str(), "%d.%d.%d.%d", &parts[0], &parts[1], &parts[2],
               &parts[3]) != 4)
    return false;
...
static bool GetFileVersionData(const char *path, u64 *out,
                               std::string *outProductName) {
...
const IMAGE_SECTION_HEADER *GetImageSection(const u8 *base, const char *name) {
  ...
  const IMAGE_NT_HEADERS *ntHeader =
      (IMAGE_NT_HEADERS *)(base + dosHeader->e_lfanew);
...
bool HasImportedLibrary(const u8 *base, const char *name) {
  ...
  auto *importDir = (const IMAGE_DATA_DIRECTORY *)&ntHeader->OptionalHeader
  ...
  if (const auto *importTable = (const IMAGE_IMPORT_DESCRIPTOR *)LookupRVA(
...
static bool IsWinStoreImage(const u8 *base) {
  // .xbld section shows up in the steam version, need to look for some other
  // import
  return false;
}
...
bool ScanEXE(const char *path, ProcHookInfo *hookInfo) {
  ...
  if (mapping) {
    const u8 *fileBase =
        (const u8 *)MapViewOfFile(mapping, FILE_MAP_READ, 0, 0, 0);
    if (fileBase) {
...
bool IdentifyEXE(const char *procName, bool isEditor, std::string *dllSuffix,
  ...
  const u64 kCurVersion = (u64(GET_EXE_VERSION_MAJOR(RUNTIME_VERSION)) << 48) |
  ...
  u32 versionInternal =
      MAKE_EXE_VERSION(version >> 48, version >> 32, version >> 16);
...
#error unknown runtime type
    ...
    if (foundProcTypeName && (hookInfo->procType != expectedProcType)) {
      // different build
      PrintLoaderError("This version of SFSE is compatible with the %s version "
                       "of the game.\n"
                       "You have the %s version of the game. Please download "
                       "the correct version from the website.\n"
                       "Runtime: %d.%d.%d\n"
                       "SFSE: %d.%d.%d",
                       ...
                       SFSE_VERSION_INTEGER, SFSE_VERSION_INTEGER_MINOR,
                       SFSE_VERSION_INTEGER_BETA);
  ...
  if (version < kCurVersion) {
    ...
    if (versionInternal == CURRENT_RELEASE_RUNTIME)
      PrintLoaderError("You are using the version of SFSE intended for the "
                       "Steam beta branch (%d.%d.%d).\n"
                       "Download and install the non-beta branch version (%s) "
                       "from http://sfse.silverlock.org/.",
                       SFSE_VERSION_INTEGER, SFSE_VERSION_INTEGER_MINOR,
                       SFSE_VERSION_INTEGER_BETA, CURRENT_RELEASE_SFSE_STR);
    else
      PrintLoaderError("You are using Starfield version %d.%d.%d, which is out "
                       "of date and incompatible with this version of SFSE "
                       "(%d.%d.%d). Update to the latest beta version.",
                       GET_EXE_VERSION_MAJOR(versionInternal),
                       GET_EXE_VERSION_MINOR(versionInternal),
                       GET_EXE_VERSION_BUILD(versionInternal),
                       SFSE_VERSION_INTEGER, SFSE_VERSION_INTEGER_MINOR,
    ...
    PrintLoaderError("You are using Starfield version %d.%d.%d, which is out "
                     "of date and incompatible with this version of SFSE "
                     "(%d.%d.%d). Update to the latest version.",
                     GET_EXE_VERSION_MAJOR(versionInternal),
                     GET_EXE_VERSION_MINOR(versionInternal),
                     GET_EXE_VERSION_BUILD(versionInternal),
                     SFSE_VERSION_INTEGER, SFSE_VERSION_INTEGER_MINOR,
    ...
    PrintLoaderError("You are using a newer version of Starfield than this "
                     "version of SFSE supports.\n"
                     "If this version just came out, please be patient while "
                     "we update our code.\n"
                     "In the meantime, please check http://sfse.silverlock.org "
                     "for updates.\n"
    ...
    char versionStr[256];
    sprintf_s(versionStr, "%d_%d_%d", GET_EXE_VERSION_MAJOR(versionInternal),
              GET_EXE_VERSION_MINOR(versionInternal),
              GET_EXE_VERSION_BUILD(versionInternal));
