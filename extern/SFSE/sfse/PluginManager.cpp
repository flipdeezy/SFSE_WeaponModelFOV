...
=======
static const SFSEInterface g_SFSEInterface = {PACKED_SFSE_VERSION,
                                              RUNTIME_VERSION,
                                              SFSEInterface::kInterfaceVersion,
...
static const SFSETrampolineInterface g_SFSETrampolineInterface = {
    SFSETrampolineInterface::kInterfaceVersion, AllocateFromSFSEBranchPool,
    AllocateFromSFSELocalPool};
...
PluginManager::~PluginManager() { deinit(); }
...
void PluginManager::installPlugins(u32 phase) {
  for (size_t i = 0; i < m_plugins.size(); i++) {
    ...
    _MESSAGE("%sloading plugin \"%s\"", (phase == kPhase_Preload) ? "pre" : "",
             plugin.version.name);
    ...
    if (plugin.handle) {
      plugin.load[phase] = (_SFSEPlugin_Load)GetProcAddress(
          plugin.handle,
          (phase == kPhase_Preload) ? "SFSEPlugin_Preload" : "SFSEPlugin_Load");
      if (plugin.load) {
        const char *loadStatus = nullptr;
...
void PluginManager::loadComplete() {
  for (size_t i = 0; i < m_plugins.size(); i++) {
    ...
    _MESSAGE("plugin %s (%08X %s %08X) %s (handle %d)", plugin.dllName.c_str(),
             plugin.version.dataVersion, plugin.version.name,
             plugin.version.pluginVersion, "loaded correctly",
             plugin.internalHandle);
  }
  ...
  // alert any listeners that plugin load has finished
  dispatchMessage(0, SFSEMessagingInterface::kMessage_PostLoad, nullptr, 0,
                  nullptr);
  // second post-load dispatch
  dispatchMessage(0, SFSEMessagingInterface::kMessage_PostPostLoad, nullptr, 0,
                  nullptr);
}

void PluginManager::deinit() {
  for (LoadedPluginList::iterator iter = m_plugins.begin();
...
const PluginInfo *PluginManager::infoByName(const char *name) const {
  for (LoadedPluginList::const_iterator iter = m_plugins.begin();
       iter != m_plugins.end(); ++iter) {
    const LoadedPlugin *plugin = &(*iter);
...
PluginHandle PluginManager::getPluginHandle(void) {
  ASSERT_STR(s_currentPluginHandle,
             "A plugin has called SFSEInterface::GetPluginHandle outside of "
             "its Query/Load handlers");
...
void PluginManager::scanPlugins(void) {
  ...
  for (DirectoryIterator iter(m_pluginDirectory.c_str(), "*.dll"); !iter.done();
       iter.next()) {
    std::string pluginPath = iter.fullPath();
    ...
    HMODULE resourceHandle = (HMODULE)LoadLibraryEx(
        pluginPath.c_str(), nullptr, LOAD_LIBRARY_AS_IMAGE_RESOURCE);
    if (resourceHandle) {
      if (is64BitDLL(resourceHandle)) {
        auto *version =
            (const SFSEPluginVersionData *)getResourceLibraryProcAddress(
                resourceHandle, "SFSEPlugin_Version");
        if (version) {
          plugin.version = *version;
          sanitize(&plugin.version);
          ...
          if (!loadStatus) {
            ...
            plugin.hasLoad = getResourceLibraryProcAddress(
                                 resourceHandle, "SFSEPlugin_Load") != nullptr;
            plugin.hasPreload =
                getResourceLibraryProcAddress(resourceHandle,
                                              "SFSEPlugin_Preload") != nullptr;
...
const char *PluginManager::checkAddressLibrary(void) {
  ...
  _snprintf_s(fileName, 256, "Data\\SFSE\\Plugins\\versionlib-%d-%d-%d-%d.bin",
              GET_EXE_VERSION_MAJOR(RUNTIME_VERSION),
              GET_EXE_VERSION_MINOR(RUNTIME_VERSION),
              GET_EXE_VERSION_BUILD(RUNTIME_VERSION), 0);
...
const char *PluginManager::safeCallLoadPlugin(LoadedPlugin *plugin,
                                              const SFSEInterface *sfse,
                                              u32 phase) {
  __try {
    if (!plugin->load[phase](sfse)) {
      return "reported as incompatible during load";
...
static const MinVersionEntry kMinVersionList[] = {{nullptr, 0, nullptr}};

const char *
PluginManager::checkPluginCompatibility(const SFSEPluginVersionData &version) {
  __try {
    // basic validity
    if (!version.dataVersion) {
      return "disabled, bad version data";
...
void PluginManager::logPluginLoadError(const LoadedPlugin &pluginSrc,
                                       const char *errStr, u32 errCode,
                                       bool isError) {
  LoadedPlugin plugin = pluginSrc;
  ...
  _MESSAGE("plugin %s (%08X %s %08X) %s %d (handle %d)", plugin.dllName.c_str(),
           plugin.version.dataVersion, plugin.version.name,
           plugin.version.pluginVersion, plugin.errorState, plugin.errorCode,
           s_currentPluginHandle);
}
...
// some plugins have non-descriptive names resulting in bad bug reports
static const BetterPluginName kBetterPluginNames[] = {{nullptr, nullptr}};
...
void PluginManager::reportPluginErrors() {
  ...
  // With this plugin DLL load error, the thread of prophecy is severed. Update
  // your plugins to restore the weave of fate, or persist in the doomed world
  // you have created

  std::string message =
      "A DLL plugin has failed to load correctly. If a new version of "
      "Starfield was just released, the plugin needs to be updated. Please "
      "check the mod's webpage for updates.\n";
  ...
  message += "\n\nContinuing to load may result in lost save data or other "
             "undesired behavior.";
  message += "\nExit game? (yes highly suggested)";
...
void PluginManager::updateAddressLibraryPrompt() {
  int result = MessageBox(
      ...
      "Library. Either this is a new install, or Starfield was just updated. "
      "Visit the Address Library webpage for updates?",
      "SFSE Plugin Loader", MB_YESNO);

  if (result == IDYES) {
    //		ShellExecute(0, nullptr,
    //"https://www.nexusmods.com/starfield/mods/123456789", nullptr, nullptr,
    //0);
    TerminateProcess(GetCurrentProcess(), 0);
  }
>>>>>>> 11ad18d6b375ff01709996cfabff128af874a1fc
...
bool PluginManager::registerListener(
    PluginHandle listener, const char *sender,
    SFSEMessagingInterface::EventCallback handler) {
  // because this can be called while plugins are loading, gotta make sure
  // number of plugins hasn't increased
  u32 numPlugins = g_pluginManager.numPlugins() + 1;
  if (s_pluginListeners.size() < numPlugins) {
    s_pluginListeners.resize(
        numPlugins + 5); // add some extra room to avoid unnecessary re-alloc
  }

  _MESSAGE("registering plugin listener for %s at %u of %u", sender, listener,
           numPlugins);
  ...
  if (sender) {
    ...
    // is listener already registered?
    for (std::vector<PluginListener>::iterator iter =
             s_pluginListeners[target].begin();
         iter != s_pluginListeners[target].end(); ++iter) {
      if (iter->listener == listener) {
        return true;
      }
    }
    ...
    // register listener to every loaded plugin
    u32 idx = 0;
    for (PluginListeners::iterator iter = s_pluginListeners.begin();
         iter != s_pluginListeners.end(); ++iter) {
      // don't add the listener to its own list
      if (idx && idx != listener) {
        bool skipCurrentList = false;
        for (std::vector<PluginListener>::iterator iterEx = iter->begin();
             iterEx != iter->end(); ++iterEx) {
          // already registered with this plugin, skip it
          if (iterEx->listener == listener) {
            skipCurrentList = true;
            break;
...
bool PluginManager::dispatchMessage(PluginHandle sender, u32 messageType,
  ...
  if (!senderName)
    return false;
  for (std::vector<PluginListener>::iterator iter =
           s_pluginListeners[sender].begin();
       iter != s_pluginListeners[sender].end(); ++iter) {
    SFSEMessagingInterface::Message msg;
    msg.data = data;
    msg.type = messageType;
    msg.sender = senderName;
    ...
    if (target != kPluginHandle_Invalid) // sending message to specific plugin
    {
      ...
      _DMESSAGE("sending message type %u to plugin %u", messageType,
                iter->listener);
      iter->handleMessage(&msg);
      numRespondents++;
    }
...
inline void *BranchTrampolineManager::allocate(PluginHandle plugin,
                                               size_t size) {
  auto mem = m_trampoline.allocate(size);
  if (mem) {
    std::lock_guard<decltype(m_lock)> locker(m_lock);
