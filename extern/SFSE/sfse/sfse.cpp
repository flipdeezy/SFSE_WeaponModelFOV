...
=======
typedef char *(*__get_narrow_winmain_command_line)();
__get_narrow_winmain_command_line _get_narrow_winmain_command_line_Original =
    NULL;
...
void installBaseHooks(void) {
  ...
  // fetch functions to hook
  auto *initterm = (__initterm_e *)getIATAddr(
      exe, "api-ms-win-crt-runtime-l1-1-0.dll", "_initterm_e");
  auto *cmdline = (__get_narrow_winmain_command_line *)getIATAddr(
      exe, "api-ms-win-crt-runtime-l1-1-0.dll",
  ...
  if (cmdline) {
    _get_narrow_winmain_command_line_Original = *cmdline;
    safeWrite64(uintptr_t(cmdline),
                u64(__get_narrow_winmain_command_line_Hook));
  } else {
    _ERROR("couldn't find _get_narrow_winmain_command_line");
  }
}
...
void SFSE_Preinit() {
  ...
  _MESSAGE("SFSE runtime: initialize (version = %d.%d.%d %08X %04d-%02d-%02d "
           "%02d:%02d:%02d, os = %s)",
  ...
  if (!g_branchTrampoline.create(1024 * 64)) {
    _ERROR("couldn't create branch trampoline. this is fatal. skipping "
           "remainder of init process.");
    return;
  }

  if (!g_localTrampoline.create(1024 * 64, g_moduleHandle)) {
    _ERROR("couldn't create codegen buffer. this is fatal. skipping remainder "
           "of init process.");
    return;
  }
...
extern "C" {
void StartSFSE() { installBaseHooks(); }
