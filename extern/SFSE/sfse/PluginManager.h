...
public:
  ...
  static bool dispatchMessage(PluginHandle sender, u32 messageType, void *data,
                              u32 dataLen, const char *receiver);
  static bool registerListener(PluginHandle listener, const char *sender,
...
private:
  ...
  const char *safeCallLoadPlugin(LoadedPlugin *plugin,
                                 const SFSEInterface *sfse, u32 phase);

  void sanitize(SFSEPluginVersionData *version);
  const char *checkPluginCompatibility(const SFSEPluginVersionData &version);
  const char *checkAddressLibrary(void);

  void logPluginLoadError(const LoadedPlugin &plugin, const char *errStr,
                          u32 errCode = 0, bool isError = true);
  void reportPluginErrors();
  void updateAddressLibraryPrompt();
